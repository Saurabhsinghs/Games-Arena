...
class App extends Component {
  ...
  render() {
    var GameList = this.state.games.map(function (game, index) {
      return <GameItem key={index} data={game} />;
    }, this);
        ...
        <center>
          <div className="margin-3">
            ...
            <RaisedButton
              className="margin-3"
              label={
                (this.state.sort === "" && "Sort by Score") ||
                (this.state.sort === "asc" && "Sort by Score(High to Low)") ||
                (this.state.sort === "dsc" && "Sort by Score (Low to High)")
              }
              primary={true}
